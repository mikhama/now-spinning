...
- Detect playback start only after RPM reaches `SPINNING_RPM_THRESHOLD = 5500` and remains in the post-threshold tonearm delay window until `TONEARM_DELAY_AUTO = 9726` milliseconds has elapsed.
- Detect playback stop when RPM falls below `SPINNING_RPM_THRESHOLD = 5500`.
